...
fn check_disposition(metadata: &HashMap<String, String>, key: &str) -> bool {
    metadata.get(&format!("disposition:{}", key))
...
fn display_title(language: Option<&str>, codec: Option<&str>, codec_type: &str, channels: Option<i32>) -> Option<String> {
    ...
    if codec_type == "Audio" {
        if let Some(ch) = channels {
            let layout = if ch >= 8 { "7.1" } else if ch >= 6 { "5.1" } else { "Stereo" };
...
pub fn probe_media(url: &str) -> Result<jellyfin::MediaSourceInfo> {
    ...
    tracing::debug!(count = stream_infos.len(), "found stream_infos from ez-ffmpeg");
    ...
    for info in stream_infos.iter() {
        let stream = match info {
            StreamInfo::Video {
                index, codec_name, width, height, bit_rate, fps, metadata, ..
            } => {
                ...
                jellyfin::MediaStream {
                    ...
                    display_title: display_title(language.as_deref(), Some(codec_name), "Video", None),
                    language,
            ...
            StreamInfo::Audio {
                index, codec_name, sample_rate, nb_channels, bit_rate, metadata, ..
            } => {
                let language = metadata.get("language").cloned();
                jellyfin::MediaStream {
                    ...
                    is_default: Some(check_disposition(metadata, "default")),
                    is_forced: Some(check_disposition(metadata, "forced")),
                    display_title: display_title(language.as_deref(), Some(codec_name), "Audio", nonzero(*nb_channels)),
                    language,
                    title: metadata.get("title").cloned(),
                    ..Default::default()
                }
            ...
            StreamInfo::Subtitle {
                ...
                jellyfin::MediaStream {
                    ...
                    is_default: Some(check_disposition(metadata, "default")),
                    is_forced: Some(check_disposition(metadata, "forced")),
                    display_title: display_title(language.as_deref(), Some(codec_name), "Subtitle", None),
                    language,
                    title: metadata.get("title").cloned(),
                    ..Default::default()
                }
