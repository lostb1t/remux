...
pub async fn search_hints(
    ...
    let hints: Vec<api::SearchHint> =
        result.items.into_iter().map(item_to_hint).collect();
...
fn item_to_hint(item: api::BaseItemDto) -> api::SearchHint {
    api::SearchHint {
        ...
        name: item.name,
        type_: item.type_,
        primary_image_tag: item.image_tags.and_then(|t| t.primary),
        production_year: item.production_year,
        run_time_ticks: item.run_time_ticks,
        is_folder: Some(item.is_folder),
        media_type: Some(item.media_type.to_string()),
