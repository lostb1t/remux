                ...
                .map(|(i, sort)| {
                    ...
                    let col = match sort {
                        ...
                        api::ItemSortBy::PopularityAllTime => {
                            ...
                            "COALESCE(\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'all' AND pa.period_key = 'all'),\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'yearly' ORDER BY pa.period_key DESC LIMIT 1),\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'monthly' ORDER BY pa.period_key DESC LIMIT 1),\
                               0) DESC"
                                .to_string()
                        }
                        api::ItemSortBy::PopularityDay => {
                            ...
                            "COALESCE(\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'daily' AND pa.period_key = date('now')),\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'daily' ORDER BY pa.period_key DESC LIMIT 1),\
                               0) DESC"
                                .to_string()
                        }
                        api::ItemSortBy::PopularityWeek => {
                            ...
                            "COALESCE(\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'weekly' AND pa.period_key = strftime('%Y-W%W', 'now')),\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'weekly' ORDER BY pa.period_key DESC LIMIT 1),\
                               0) DESC"
                                .to_string()
                        }
                        api::ItemSortBy::PopularityMonth => {
                            ...
                            "COALESCE(\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'monthly' AND pa.period_key = strftime('%Y-%m', 'now')),\
                               (SELECT pa.avg FROM popularity_agg pa WHERE pa.external_id = 'tmdb:' || CAST(json_extract(media.external_ids, '$.tmdb') AS TEXT) AND pa.source = 'tmdb' AND pa.period = 'monthly' ORDER BY pa.period_key DESC LIMIT 1),\
                               0) DESC"
                                .to_string()
                        }
