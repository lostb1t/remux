...
// Strip "provider:" prefix from item-detail tag link labels (display only).
// The tag's href is left untouched, so clicking still filters by the raw
// "provider:..." value stored server-side. Mirrors the "Recently Added in" stripper.
(function () {
  var PREFIX = 'provider:';
  ...
  function processRoot(root) {
    if (!root.querySelectorAll) return;
    var els = root.querySelectorAll('.itemTags a');
    for (var i = 0; i < els.length; i++) clean(els[i]);
    if (root.matches && root.matches('.itemTags a')) clean(root);
  }
