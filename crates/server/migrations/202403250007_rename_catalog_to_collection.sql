...
-- Recreate media table with:
--   kind constraint: 'catalog' -> 'collection'
--   columns: catalog_kind -> collection_kind, catalog_media_kind -> collection_media_kind
...
-- Copy data, converting kind='catalog' to kind='collection'
INSERT INTO media_new (
    ...
    collection_kind, collection_media_kind,
    created_at, updated_at, refreshed_at
)
SELECT
    id,
    title,
    CASE kind WHEN 'catalog' THEN 'collection' ELSE kind END,
    imdb_id, aio_id, series_imdb_id, parent_id,
    idx, parent_idx, released_at, runtime, rating_critic, rating_audience,
    certification, poster, logo, backdrop, description, trailers,
    url, probe_data, remote_data, promoted,
    catalog_kind,
