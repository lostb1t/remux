...
fn task_info(
    handler: &TaskView,
    triggers: Vec<api::db::TaskTrigger>,
    last_result: Option<api::db::TaskResult>,
) -> api::TaskInfo {
    ...
    let (last_execution_result, last_execution_date) = match last_result {
        Some(r) => {
            let result = api::TaskResult {
                status: Some(
                    match r.status {
                        api::db::TaskResultStatus::Completed => "Completed",
                        api::db::TaskResultStatus::Failed => "Failed",
                        api::db::TaskResultStatus::Stopped => "Stopped",
                    }
                    .to_string(),
                ),
...
fn trigger_to_cron(t: &api::TaskTriggerInfo) -> Option<String> {
    let kind = t
        .r#type
        .as_deref()
...
pub async fn update_task_triggers(
    ...
    State(state): State<AppState>,
    _session: auth::AdminSession,
    Json(trigger_infos): Json<Vec<api::TaskTriggerInfo>>,
) -> Result<impl axum::response::IntoResponse> {
    let triggers: Vec<api::db::TaskTrigger> = trigger_infos
        .into_iter()
        .map(|t| api::db::TaskTrigger {
