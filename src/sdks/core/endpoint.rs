...
use http::{self, HeaderMap, Method, Request, header};
use serde::de::DeserializeOwned;
use std::{borrow::Cow, collections::HashMap};

use super::{ApiError, Client, QueryParams, RestClient, query};
...
pub trait Endpoint {
    ...
    async fn query(&self, client: &RestClient) -> Result<Self::Output, ApiError> {
        let res = client
            .request(
                self.method(),
                Some(self.endpoint()),
        ...
        if !status.is_success() {
            return Err(ApiError::Other(format!(
                "something went wrong, status: {}, res: {}",
                status,
                res.text().await?
            )));
        }
